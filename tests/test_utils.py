...
from src.utils import greeting_by_time_of_day, top_transaction_payment_amount, exchange_rates, get_card_data, get_actions_data
...
def test_get_card_data():
    data = {
        'Сумма операции': [-100, -200, -300, 100],
        'Номер карты': ['*3456', '*3456', '*3456', '*4444']
    }
    df = pd.DataFrame(data)
...
def test_get_actions_data(mock_get):
    ...
    expected_result = {
        "stock_prices": [
            {"stock": "AAPL", "price": 155.50},
            {"stock": "AMZN", "price": 155.50},
            {"stock": "GOOGL", "price": 155.50},
            {"stock": "MSFT", "price": 155.50},
            {"stock": "TSLA", "price": 155.50},
        ]
    }
